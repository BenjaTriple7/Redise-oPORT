...
function renderHero(hero) {
    ...
    <section class="case-hero">
        ...
        <div class="case-hero-visual">
            <img src="${hero.imagen}" class="hero-mockup reveal-img visible" alt="Preview del rediseño">
        </div>
    </section>
    `;
}
...
function renderProblema(problema) {
    const parrafosHTML = problema.parrafos
        .map(p => `<p>${p}</p>`)
        .join('');

    const hallazgosHTML = problema.hallazgos
        .map(h => `
            <div class="finding">
    ...
    <section class="section-wide" id="problema">
        <div class="case-grid">
            <div class="case-text" data-index="01">
                ...
                <p class="case-note">${problema.nota}</p>
                ${parrafosHTML}
...
function renderAnalisis(analisis) {
    const parrafosHTML = analisis.parrafos
        .map(p => `<p>${p}</p>`)
        .join('');

    return `
    <section id="analisis" style="--analysis-bg: url('${analisis.backgroundImage}');">
        <div class="analysis-overlay">
            <p class="small">${analisis.label}</p>
            <h2>${analisis.titulo}</h2>
            <p class="case-note">${analisis.nota}</p>
            ${parrafosHTML}
        </div>
    </section>
    `;
...
function renderProceso(proceso) {
    ...
    const parrafosHTML = proceso.parrafos
        .map(p => `<p>${p}</p>`)
        .join('');
    ...
    <section class="section-wide" id="proceso">
        ...
        <div class="case-grid">
            <div class="case-text" data-index="03">
                ...
                <p class="case-note">${proceso.nota}</p>
                ${parrafosHTML}
            </div>
        </div>
    </section>
...
function renderSolucion(solucion) {
    const parrafosHTML = solucion.parrafos
        .map(p => `<p>${p}</p>`)
        .join('');
    ...
    <section id="solucion" class="section-wide">
        <div class="case-grid reverse">
            <div class="case-text" data-index="04">
                ...
                <p class="case-note">${solucion.nota}</p>
                ${parrafosHTML}
            </div>
            <div class="compare-wrapper">
                <div class="compare-container" id="compareContainer">
                    <img src="${solucion.comparacion.antes}" class="compare-after" alt="Después">
                    <div class="compare-before" id="compareBefore">
                        <img src="${solucion.comparacion.despues}" alt="Antes">
                    </div>
                ...
                <div class="compare-labels">
                    <span>${solucion.comparacion.labelAntes}</span>
                    <span>${solucion.comparacion.labelDespues}</span>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="approval-inner">
            ...
            <div class="approval-content">
                ...
                <div class="approval-text">
                    <p class="approval-eyebrow small">${validacion.eyebrow}</p>
...
function renderResultado(resultado) {
    ...
    <section class="section-narrow" id="resultado">
        ...
        <div style="display:flex; flex-wrap:wrap; gap:0.75rem;">
            ...
            <a href="${resultado.sitio}" target="_blank" class="btn btn-ghost " data-delay="3">Ver sitio en vivo ↗</a>
        </div>
    </section>
    `;
}